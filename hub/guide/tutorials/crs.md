...
<img src="/images/tutorials/uqm_map_400px.png" style="border: 1px solid #ccc; border-radius: 5px" /><br/>
</div>
...
<div class="flex items-center justify-center">
<img src="/images/tutorials/uqm_map_detail.png" style="border: 1px solid #ccc; border-radius: 5px" /><br/>
</div>
...
<RenderMap @mapReady="onFirstExampleReady" />
...
```
<RenderMap @mapReady="onSecondExampleReady" />
...
<RenderMap @mapReady="onThirdExampleReady" />

<script setup>
import { withBase } from 'vitepress';
const onFirstExampleReady = (map, L) => {
    map.options.crs = L.CRS.Simple;

	const bounds = [[0, 0], [1000, 1000]];
	const image = new L.ImageOverlay(withBase('/images/tutorials/uqm_map_full.png'), bounds).addTo(map);

	map.fitBounds(bounds);
};

const onSecondExampleReady = (map, L) => {
    map.options.crs = L.CRS.Simple;
	...
	const sol = new L.LatLng(145, 175);
	const marker = new L.Marker(sol).addTo(map);

	map.setView([70, 120], 1);
};

const onThirdExampleReady = (map, L) => {
	...
	const mSol = new L.Marker(sol).addTo(map).bindPopup('Sol');
	const mMizar = new L.Marker(mizar).addTo(map).bindPopup('Mizar');
	const mKruegerZ = new L.Marker(kruegerZ).addTo(map).bindPopup('Krueger-Z');
	const mDeneb = new L.Marker(deneb).addTo(map).bindPopup('Deneb');

	const travel = new L.Polyline([sol, deneb]).addTo(map);
